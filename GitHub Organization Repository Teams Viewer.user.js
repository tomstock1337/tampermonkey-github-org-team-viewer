...
(function() {
	...
	const pageOrgRepoRegex = /(https?:\/\/(www\.)?github\.com\/orgs\/)([^\/]*)\/repositories\/?/;
	const pageOrgProfileRegex = /(https?:\/\/(www\.)?github\.com\/)([^\/]*)\/?$/;
	const pageOrgTeamProfileRegex = /(https?:\/\/(www\.)?github\.com\/orgs\/)([^\/]*)\/teams\/([^\/]*)\/repositories\/?/;
			...
			if (stage1 !== 'Finished') {
				...
				function stage3Start() {
					if (stage2cntTeamsProcessed != orgTeams.length) {
						...
						var orgReposList = $("ul li a[data-hovercard-type='repository']", orgRepos);
						$.each(orgReposList, (i3) => {
							var repoURL = $(orgReposList[i3]).attr('href');
							var repoList = $("ul li div[itemType='http://schema.org/Code']", orgRepos);
							var repoTeams = [];
							var pills = $('<div></div>');
